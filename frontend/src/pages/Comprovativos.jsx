...
import { FileText, Check, X, RotateCcw, Eye } from "lucide-react";
      ...
      {preview && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setPreview(null)}>
          <div className="max-h-[85vh] max-w-2xl overflow-auto rounded-xl bg-white p-4" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 font-semibold">{preview.filename}</div>
            {preview.file_data.startsWith("data:image") ? <img src={preview.file_data} alt="comprovativo" className="max-w-full rounded-lg" /> : <a href={preview.file_data} download={preview.filename} className="text-blue-600 underline">Descarregar ficheiro</a>}
          </div>
        </div>
      )}
    </>
